...
const updateItemGroup = (state, action) => {
    ...
    let newItemGroup = new ItemGroup( ...state[action.oid], ...action.updateObj);
    return {...newState, [newOid]: newItemGroup};
};

const itemGroupReducers = (state, action) => {
    switch (action.type) {
        case UPD_ITEMGROUPDEF:
...
export default itemGroupReducers;
